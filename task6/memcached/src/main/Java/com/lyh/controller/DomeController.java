...
private JobService jobService;
    ...
    public String home(Model model){
        ...
        System.out.println("展示优秀学员的人数："+num);
        ...
        if (memCachedClient.get("studentList")==null){
            logger.info("优秀学员缓存没有设置，开始设置缓存");
            for(int i=0;i<num;i++) {
                //  数组打乱后，已经是随机值，可以直接取前几个
                // Student s= students.add(studentList.get(i));
                Student s= studentList.get(i);
                System.out.println("s = " + s);
                boolean tagAdd = students.add(s);
                System.out.println("加入listt: " + tagAdd);
            }

            System.out.println("students = " + students);

            memCachedClient.set("studentList",students);
           ...
           students = ( List<Student>) memCachedClient.get("studentList");
