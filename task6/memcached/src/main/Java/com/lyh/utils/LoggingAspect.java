...
public class LoggingAspect {
    ...
    // 声明该方法是一个环绕通知
    @Around(value = "execution(* com.lyh.controller.*.*(..)) || execution(* com.lyh.service.*.*(..)) ")
    ...
    // 声明该方法是一个返回结果通知
    @AfterReturning(value = "execution(* com.lyh.service.*.*(..))",returning = "result")
    ...
    // 声明该方法是一个异常通知
    @AfterThrowing(value = "execution(* com.lyh.service.*.*(..))",throwing = "exp")
